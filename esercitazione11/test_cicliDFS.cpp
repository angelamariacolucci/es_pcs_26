...
/* Test unitari per cicli_DFS (cicli fondamentali via DFS iterativa + coalbero).
   Usiamo gli stessi grafi di test_de_pina.cpp, per poter confrontare a occhio
   i due algoritmi sullo stesso input:
...
/* Verifica che 'cycle' sia un ciclo valido in G:
   - almeno 3 nodi,
   - nodi tutti distinti,
   - ogni coppia consecutiva (con wrap-around) e' un arco di G.
*/
...
static int test_cicli_dfs_validita ()
{
    ...
    for (std::size_t i = 0; i < cicli.size(); i++) {
        if (!is_ciclo_valido(G_ciclico, cicli[i])) {
            std::cerr << "ERRORE: test_cicli_dfs_validita: ciclo " << i
                      << " non valido\n";
            return EXIT_FAILURE;
        }
    }
...
/* 4) I cicli devono essere indipendenti: ogni ciclo della base deve usare
      almeno un arco non ancora visto nei cicli precedenti. */
static int test_cicli_dfs_indipendenza ()
{
    ...
    for (const auto& cycle : cicli) {
        ...
        if (!ha_arco_nuovo) {
            std::cerr << "ERRORE: test_cicli_dfs_indipendenza: "
                      << "ciclo senza arco nuovo\n";
            return EXIT_FAILURE;
        }
    }
